...
TEMPLATE_REFRESH_INTERVAL_SECONDS = 1
hashratedict = {}
    ...
    async with ClientSession() as session:
        async with session.post(
            ...
            try:
                ...
                # The following occurs during both new blocks & new txs, or when enough time passes without changes
                # OR when force_update is True (e.g., after block submission)
                if (
                    new_block
                    or new_witness
                    or state.timestamp + TEMPLATE_REFRESH_INTERVAL_SECONDS <= ts
                    or force_update
                ):
                    # Generate coinbase #
...
if __name__ == "__main__":
    ...
    async def updateState():
        while True:
            ...
            # Force a template refresh every TEMPLATE_REFRESH_INTERVAL_SECONDS seconds
            await asyncio.sleep(TEMPLATE_REFRESH_INTERVAL_SECONDS)
